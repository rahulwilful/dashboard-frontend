...
const GameCard = (props) => {
  ...
  const handleFaceImages = () => {
    ...
    else if (props.game.toLowerCase().includes('5 card poker')) setImage(BaccaratTables[3].table)
    ...
    else if (props.game.toLowerCase().includes('dragon tiger')) setImage(BaccaratTables[8].table)
    ...
    else if (props.game.toLowerCase().includes('texas holdem')) setImage(BaccaratTables[11].table)
    ...
    >
      ...
      <div className={`d-flex flex-column gap-2 p-2 poppins-400`}>
        <div className={`text-dark ${props?.lastNumber ? '' : 'd-none'}`}>
          <span className="fw-semibold"> Winning Number</span> :{' '}
          {props?.lastNumber ? props.lastNumber : ''}
        </div>
